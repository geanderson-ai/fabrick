...
def create_client(
    ...
    # Identify active provider for logging
    provider_id = os.environ.get("AUTO_BUILD_PROVIDER", "CLAUDE").upper()
    provider_name = PROVIDERS.get(provider_id, PROVIDERS["CLAUDE"]).name
    print(f"   - API Provider: {provider_name}")

    # Ollama: override model with OLLAMA_MODEL and disable extended thinking
    # Ollama's OpenAI-compatible API doesn't support Anthropic's thinking tokens
