...
AUTH_TOKEN_ENV_VARS = [
    ...
    "OLLAMA_API_KEY",  # Ollama (optional, local LLM — no real auth needed)
]
...
def configure_sdk_authentication(config_dir: str | None = None) -> None:
    ...
    - Provider mode (AUTO_BUILD_PROVIDER set to OPENROUTER, GEMINI, etc.)
    - API Profile mode (ANTHROPIC_BASE_URL set): uses ANTHROPIC_AUTH_TOKEN
    - OAuth mode (default): uses CLAUDE_CODE_OAUTH_TOKEN

    In API profile and Provider modes, explicitly removes CLAUDE_CODE_OAUTH_TOKEN
    from the environment because the SDK gives OAuth priority over API keys.
    ...
    """
    provider_id = os.environ.get("AUTO_BUILD_PROVIDER", "OPENROUTER").upper()
    api_profile_mode = bool(os.environ.get("ANTHROPIC_BASE_URL", "").strip())

    # Mode 0: Ollama (local LLM — no real authentication needed)
    if provider_id == "OLLAMA":
        from core.provider_config import get_ollama_base_url, get_ollama_model

        ollama_base_url = get_ollama_base_url()
        ollama_model = get_ollama_model()

        # Ollama exposes an OpenAI-compatible API at /v1
        # The SDK talks to it via ANTHROPIC_BASE_URL + a dummy auth token
        os.environ["ANTHROPIC_BASE_URL"] = ollama_base_url
        # Ollama doesn't validate auth tokens, but the SDK requires one to be set
        os.environ["ANTHROPIC_AUTH_TOKEN"] = os.environ.get(
            "OLLAMA_API_KEY", "ollama-local"
        )
        # Store resolved model so downstream code can read it
        os.environ["ANTHROPIC_MODEL"] = ollama_model

        # Clear OAuth token to prevent SDK from trying Claude auth
        os.environ.pop("CLAUDE_CODE_OAUTH_TOKEN", None)
        logger.info(
            f"Using Ollama authentication (model={ollama_model}, url={ollama_base_url})"
        )
        return

    # Mode 1: Alternative Provider (OPENROUTER, GEMINI, etc.)
    if provider_id != "CLAUDE" and provider_id in PROVIDERS:
        provider = PROVIDERS[provider_id]
        token = os.environ.get(provider.auth_env_var)

        if not token:
            raise ValueError(
                f"Provider {provider_id} active but {provider.auth_env_var} is not set. "
                f"Run: python auto-claude/run.py --setup {provider_id}"
            )

        # Map provider token to ANTHROPIC_AUTH_TOKEN which the SDK expects
        os.environ["ANTHROPIC_AUTH_TOKEN"] = token

        # Set base URL if not already explicitly overridden
        if provider.base_url and not os.environ.get("ANTHROPIC_BASE_URL"):
            os.environ["ANTHROPIC_BASE_URL"] = provider.base_url

        # Clear OAuth token to ensure SDK uses the provider token
        os.environ.pop("CLAUDE_CODE_OAUTH_TOKEN", None)
        logger.info(f"Using {provider.name} authentication")
        return

    # Mode 2: API Profile mode
    if api_profile_mode:
        ...
        # Mode 3: OAuth mode (default Anthropic)
