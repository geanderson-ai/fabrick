...
def resolve_model_id(model: str) -> str:
    """
    Resolve a model shorthand to a full model ID based on the active provider.
    
    Priority:
    1. Environment variable override
    2. Provider-specific model mapping
    3. Global hardcoded MODEL_ID_MAP
    ...
    # Ollama: always resolve to OLLAMA_MODEL, ignore Claude shorthands
    if provider_id == "OLLAMA":
        from core.provider_config import get_ollama_model
        # Check provider-specific model mappings first (e.g. "qwen3" -> "qwen3:8b")
        if model in provider.models:
            return provider.models[model]
        # For any unrecognized shorthand (e.g. "sonnet", "haiku"), return the Ollama default
        if not model or model == "default" or model in MODEL_ID_MAP:
            return get_ollama_model()
        return model

    # Check for environment variable override (e.g. ANTHROPIC_DEFAULT_SONNET_MODEL)
    env_var_map = {
    ...
    if model in env_var_map:
        env_val = os.environ.get(env_var_map[model])
        if env_val:
            return env_val
    ...
    # If no specific model provided (empty string or None), use provider default
    if not model or model == "default":
        return provider.default_model

    # Global hardcoded map fallback
...
def get_phase_thinking_budget(
    ...
    Returns:
        Token budget or None for no extended thinking
    """
    # Ollama models don't support Anthropic's extended thinking
    provider_id = os.environ.get("AUTO_BUILD_PROVIDER", "CLAUDE").upper()
    if provider_id == "OLLAMA":
        return None
